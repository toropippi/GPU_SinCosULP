...
__kernel void sweep_trig_4outs(
    ...
    // 規格準拠の精度（ソフト/高速パスは実装依存）
    const float s  = sin(x);
    const float c  = cos(x);

    // ネイティブ経路（通常は SFU/装置ネイティブ命令。精度は実装依存）
    const float sn = native_sin(x);
    const float cn = native_cos(x);

    // 書き込み順は gid 昇順 = ビットパターン昇順
    out_sin[gid]        = s;
    out_cos[gid]        = c;
    out_native_sin[gid] = sn;
    out_native_cos[gid] = cn;
